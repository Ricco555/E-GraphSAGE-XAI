...
class NumericArtifacts:
    ...
    # NEW: dimensions at different stages
    dim_before_var: int                # after scaling, before zero-variance prune
    dim_after_var: int                 # after zero-variance prune, before corr prune
    dim_after_corr: int                # final dim after corr prune
...
def fit_numeric_transform(
    ...
    # dim BEFORE any pruning (just scaled)
    dim_before_var = X.shape[1]

    # 4) zero-variance drop
    X, keep_var = _drop_zero_variance(X, eps=1e-12)
    dim_after_var = X.shape[1]
    ...
    arts = NumericArtifacts(
        ...
        dim_before_var=int(dim_before_var),
        dim_after_var=int(dim_after_var),
        dim_after_corr=int(dim_after_corr),
    )
